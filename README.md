...
`soundfield` 是一个单文件静态网页声级计应用，入口文件为 `soundfield.html`。它面向移动端浏览器和桌面浏览器，提供实时分贝估算、A/C/Z 计权切换、快/慢响应、波形/频谱/1/3 倍频程/趋势可视化、录音/录像取证、GPS 与结构化测点标记、历史记录、CSV/PDF 导出等能力。
...
http://localhost:8080/soundfield.html  # 直接打开声级记录工具
```
...
soundfield/
  ...
  app.html           # Web/PWA 工具入口页，跳转到 soundfield.html
  soundfield.html    # 单文件应用：HTML + CSS + JavaScript
...
新增的 `assets/` 文件是方案 B/C 的第一步拆分。后续继续拆分时，优先把内联 CSS/JS 逐步迁移到 `assets/`，每次迁移后保持 `soundfield.html` 可直接运行。
...
- 保持 `soundfield.html` 可直接运行，不引入必须构建的工具链，除非明确迁移为正式项目。
